...
export const attendanceService = {
  ...
  async getEmployeeAttendance(employeeId, filters = {}) {
    try {
      ...
      throw new Error(error.response?.data?.error?.message || 'Failed to fetch employee attendance');
    }
  },
  ...
  async getAbsentEmployees(filters = {}) {
    try {
      ...
      const url = params.toString()
        ? `/attendance/absent-employees?${params.toString()}` 
        : '/attendance/absent-employees';      const response = await apiClient.get(url);
      
      if (response.data.success) {
        return {
          absentEmployees: response.data.data.absentEmployees || [],
      ...
      throw new Error(error.response?.data?.error?.message || 'Failed to fetch absent employees');
    }
  },
  ...
  async generateReport(reportParams) {
    try {
      ...
      if (reportParams.department) params.append('department', reportParams.department);
  ...
  async exportAttendance(exportParams) {
    try {
      ...
      if (exportParams.department) params.append('department', exportParams.department);
